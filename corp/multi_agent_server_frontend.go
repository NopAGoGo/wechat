...
func (frontend *MultiAgentServerFrontend) ServeHTTP(c *gin.Context) {
	queryValues, err := url.ParseQuery(c.Request.URL.RawQuery)
	if err != nil {
		frontend.errHandler.ServeError(c.Writer, c.Request, err)
		return
	}

	if interceptor := frontend.interceptor; interceptor != nil && !interceptor.Intercept(c.Writer, c.Request, queryValues) {
		return
	}
	...
	ServeHTTP(c.Writer, c.Request, queryValues, agentServer, frontend.errHandler)
}
